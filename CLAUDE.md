...
- `src/api/{Controller}Api.tsx` (e.g. `UsersApi.tsx`) files are also generated by `packages gen`, one per controller — each exports a singleton client (e.g. `UsersApi`) with one instance method per endpoint, named after the controller action (e.g. `UsersApi.GetAll()`). Don't hand-edit these. `src/api/httpClient.ts` is the shared axios instance they all call through — it's the one file in that folder that *is* hand-written and safe to edit (e.g. to add auth-header interceptors).
