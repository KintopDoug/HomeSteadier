...
- **HomeSteadier.Database/Migrations/** — the DbUp SQL migration scripts themselves (embedded via `<EmbeddedResource Include="Migrations\*.sql" />`), applied transactionally and tracked in the `migrations` table. Named `NNN_PascalCaseDescription.sql` and executed in filename order.
