...
- `packages gen` — generates TypeScript request/response models under `ReactApp/src/models/` from the API's live OpenAPI document (requires the API running; defaults to `http://localhost:5128`, override via `Api:BaseUrl` / `Api__BaseUrl`).
...
5. Run `packages gen` (with the API running) to regenerate matching TypeScript request/response models in `ReactApp/src/models/`. These are also fully regenerated each run — don't hand-edit them.
