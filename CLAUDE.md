...
- **HomeSteadier.Database/Migrations/** — the DbUp SQL migration scripts themselves (embedded via `<EmbeddedResource Include="Migrations\*.sql" />`), applied transactionally and tracked in the `migrations` table. Named `NNN_PascalCaseDescription.sql` and executed in filename order. (Note: the top-level `HomeSteadier.Migrations/` directory is empty, has no csproj, and isn't in the solution — don't put migrations there.)
...
`appsettings.shared.json` at the solution root is the single source of shared config (DB name/host/port, JWT issuer/audience/expiry, refresh-cookie settings, CORS allowed origins, password-reset expiry, email sender, frontend base URL). Both `AppHost.cs` and `Homesteadier.API/Program.cs` locate it at runtime by walking up from `AppContext.BaseDirectory` looking for `HomeSteadier.slnx` (`FindSolutionRoot`), so it works regardless of which project is launched from. Secrets (`POSTGRES_PASSWORD`, `JWT_SIGNING_KEY`, `ACS_CONNECTION_STRING`) are deliberately kept out of this file and sourced from the environment instead.

**Config precedence gotcha:** `Program.cs` appends `appsettings.shared.json` *after* `CreateBuilder` has already registered the environment-variable provider, and config sources are last-wins — so the shared file would silently outrank anything AppHost/ACA injects. `Program.cs` therefore calls `builder.Configuration.AddEnvironmentVariables()` again immediately after adding the file to put env vars back on top. **Don't remove that line**: without it, any scalar key that exists in both places (`App:FrontendBaseUrl`, `Email:SenderAddress`) resolves to the local placeholder in every deployed environment. `Cors:AllowedOrigins:6/7` were never affected only because they're array indices the file doesn't define.
...
**The custom `UserStore` constrains what Identity can do.** `Homesteadier.Repository/Identity/UserStore.cs` implements only `IUserStore`, `IUserPasswordStore`, and `IUserEmailStore`, and `Program.cs` calls `AddIdentityCore<User>().AddUserStore<UserStore>()` without `.AddDefaultTokenProviders()`. Consequences worth knowing before reaching for a `UserManager` method:

- `GeneratePasswordResetTokenAsync` / `ResetPasswordAsync` need a token provider and an `IUserSecurityStampStore` — **both throw**. Password reset uses a bespoke `password_reset_tokens` table instead (below).
- `RemovePasswordAsync` + `AddPasswordAsync` is **destructive**: `SetPasswordHashAsync` coalesces null to `""`, so `AddPasswordAsync`'s null guard always trips *after* the removal has committed, leaving the account with an empty hash and no way to log in. Use `Homesteadier.API/Auth/PasswordUpdateService.cs`, which validates then hashes then saves once.
- `ChangePasswordAsync` **is** safe — its security-stamp update is a guarded no-op here, not a throw.
- There's no `IUserLockoutStore` and no `SignInManager`, so Identity lockout never engages. The only brute-force protection is the per-IP `auth` rate-limit policy (`AuthRateLimiting.cs`, 10/min, one shared bucket across every endpoint carrying `[EnableRateLimiting]`).

**Password reset** (`forgotPassword` / `resetPassword` / `changePassword` on `AuthController`) uses `password_reset_tokens`: 32 random bytes, base64url in the emailed link, only the SHA-256 digest stored. `Homesteadier.API/Auth/SecureToken.cs` holds those primitives and is shared with `RefreshTokenService` — **its hash format must stay byte-identical** (uppercase hex), or every refresh token in the database silently stops validating and every user is logged out with no error surfacing. Validation and consumption are separate steps on purpose: consuming before the password validators run would burn the user's only link on a rejected password. Issuing a new token consumes any outstanding ones, so only the newest link works. `forgotPassword` always returns 202 regardless of whether the address exists.

After any password change, all refresh tokens are revoked **before** the caller's replacement is issued (`ResetSessionsAndSignInAsync`) — reversed, the blanket revoke would kill the token just minted. Access tokens can't be revoked (no security stamp in the JWT), so other devices stay usable until theirs expires — up to `Jwt:ExpiryMinutes`.

**Email** goes through `Homesteadier.API/Email/IEmailSender.cs`. `AcsEmailSender` (Azure Communication Services) is selected when `ACS_CONNECTION_STRING` and `Email:SenderAddress` are both set; otherwise Development falls back to `LoggingEmailSender`, which writes the reset link to the API console so the flow is testable with no Azure resources. Outside Development, missing config is a startup failure.

ACS is provisioned in publish mode from `HomeSteadier.AppHost/infra/communication-services.bicep` via `AddBicepTemplate` — raw Bicep because there is no Aspire hosting integration for ACS and no `Azure.Provisioning.CommunicationServices` CDK package. It creates an Email Communication Service, an `AzureManagedDomain`, and a Communication Service with the domain linked. The connection string is written into the key vault Aspire provisions for secret outputs (the template's `keyVaultName` parameter is filled in automatically via `AzureBicepResource.KnownParameters.KeyVaultName`) and read back with `GetSecretOutput`, so the access key never lands in a deployment output or the manifest; the sender address comes back as a normal `GetOutput`. **Requires `az provider register --namespace Microsoft.Communication --wait`** on the subscription. The Azure-managed domain sends from `DoNotReply@<guid>.azurecomm.net` and is capped at roughly 100 messages/day — switch to a `CustomerManagedDomain` (two-pass, DNS-verified, like the TLS cert) before real users depend on password reset.
...
The app deploys to **Azure Container Apps (ACA)** via the **Azure Developer CLI (`azd`)**, driven by the Aspire manifest that `AppHost.cs` produces. All three services (API, React frontend, Postgres) are packaged as containers into one ACA environment.

### Deploy commands (run from `HomeSteadier.AppHost/`)

- First time on a machine/subscription: `azd auth login`, then `azd env new <name>` (prompts for subscription + region). Resource providers are **per-subscription** and must be registered up front or provisioning fails — at minimum `az provider register --namespace Microsoft.DBforPostgreSQL --wait` (Postgres capabilities return an empty version list if it's not registered, surfacing as `ParameterOutOfRange: 'Version' should be in: []`). Also register `Microsoft.App`, `Microsoft.ContainerRegistry`, `Microsoft.KeyVault`, `Microsoft.OperationalInsights`.
- Deploy everything (provision + build + push): `azd up`. Use this after any AppHost/infra change (endpoints, env vars, new resources).
- Redeploy one service's image without re-provisioning: `azd deploy <service>` (e.g. `azd deploy react-frontend`).
- Override the target resource group: `azd env set AZURE_RESOURCE_GROUP <name>` (there's no `--resource-group` flag on `azd up`). Region: `azd env set AZURE_LOCATION <region>`.

### The run-vs-publish split (the core pattern in AppHost.cs)

`AppHost.cs` branches on `builder.ExecutionContext.IsPublishMode` so local dev is untouched while publish targets Azure. Key pieces:

- **Compute environment**: `builder.AddAzureContainerAppEnvironment(...)` — no-op in run mode, the ACA target in publish.
- **Postgres**: `AddAzurePostgresFlexibleServer(...).WithPasswordAuthentication(...).RunAsContainer(...)`. Publish provisions a **managed Flexible Server** (persistent, backed up); `RunAsContainer` keeps the local pgvector container. A containerized Postgres *cannot* run in ACA — its data dir lands on an Azure Files (SMB) mount, and `initdb`'s `chmod` fails with "Operation not permitted", crash-looping the container. The admin username is pinned via a `postgres-user` parameter (`homesteadier_admin`) because Azure reserves `postgres`/`admin`, and because a random name won't match a pre-existing data volume.
- **Secrets are generated, not hand-set.** `POSTGRES_PASSWORD` and `JWT_SIGNING_KEY` are `[secret]` parameters that, in publish mode, azd generates and persists (Key Vault–backed, stable across deploys) — Postgres via `WithPasswordAuthentication` (no password arg → generated), JWT via `AddParameter(..., new GenerateParameterDefault { MinLength = 64 }, secret: true)`. In run mode both are sourced from the machine env vars so they match the CLI/local volume. The generated values never get prompted, typed, or committed.
- **The API reads Aspire's injected connection string.** `Homesteadier.API/Program.cs`'s `BuildConnectionString` prefers `ConnectionStrings:<Database:Name>` (injected by `WithReference(db)`) and only falls back to the hand-built `localhost` string for standalone `dotnet run`. Hardcoding `localhost` breaks in ACA, where Postgres is a separate service.
- **The API needs public ingress**: `api.WithExternalHttpEndpoints()` — the browser calls the API directly, so internal service-to-service reachability isn't enough. `JWT_SIGNING_KEY` is injected into the API container via `WithEnvironment` (the API reads it from the environment in `BuildJwtSettings`).
- **CORS**: the deployed frontend origin isn't known until deploy, so it's injected into the API as `Cors__AllowedOrigins__6` (`__` → `Cors:AllowedOrigins:6`, appended after the six localhost origins in `appsettings.shared.json`, which *is* present in the container because the API csproj copies it to output). Injected without a reverse `WaitFor` to avoid a startup cycle with the frontend→API reference.

### React frontend containerization

The SPA is served by **nginx** (`ReactApp/Dockerfile`, `ReactApp/nginx.conf`) via `.PublishAsDockerFile()`. Notable details:
...
- **pgvector on the managed server**: no migration creates the `vector` extension yet. When one does, Azure Flexible Server requires the extension to be allowlisted via the `azure.extensions` server parameter before `CREATE EXTENSION vector` works (the local pgvector container needs no such step).
